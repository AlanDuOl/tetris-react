...
export function Block() {
    ...
    this.moveSide = (canvas, wall, block, setBlock, moveDir) => {
        // Move block to left
    ...
    this.moveLeft = (canvas, wall, block, setBlock) => {
        let newBlock = block
    ...
    this.moveRight = (canvas, wall, block, setBlock) => {
        let newBlock = block
    ...
    this.checkMoveLeft = (canvas, wall, block) => {
    ...
    this.checkMoveRight = (canvas, wall, block) => {
        let blockCanMove = true
        let keepLooping = true
        // Check if there is a tile that can't move
        try {
    ...
    this.newSpeed = (currentBlock, setBlock) => {
        let newBlock = currentBlock
        newBlock.speed += BLOCK_DELTA_SPEED
        setBlock(newBlock)
    }

    this.rotate = (currentBlock, setBlock, wall, canvas) => {
    ...
    this.checkAvailableSpace = (block, wall, tileDim) => {
    ...
    this.getRowsAndCols = (tiles, tileDim) => {
        try {
            let tilesRows = []
            let tilesCols = []
            tiles.forEach(tile => {
    ...
    this.reset = (canvas, block, setBlock, gameReducer) => {
        let blockType = blockGetType()
        let blockProps = blockSetTiles(canvas, blockType)
    ...
    this.getRotated = (block, BLOCK_DELTA_ROTATION, tileDim) => {
        let newBlock = block
    ...
    this.getRotatedPoints = (block, rotationAngle) => {
        let newTiles = []
        try {
    ...
    this.correctPositions = (block, rotatedTiles, tileDim) => {
        let newBlock = block
    ...
    this.getTilesPosition = (block, tiles, tileDim) => {
        let newTiles = tiles
    ...
    this.getRotationPoint = (block, tileDim) => {
    ...
    this.getNewRotationAngle = (currentAngle) => {
    ...
    this.getInitialSpeed = (gameReducer) => {
