...
export function gameStart(setTimer, ctx2D, canvas, wall, setWall, block, setBlock, setGameUpdate) {
    setTimer(setInterval(gameLoop.bind(null, ctx2D, canvas, wall, setWall, block, setBlock, setGameUpdate), TIMER_SPEED))
}
...
function gameLoop(ctx2D, canvas, wall, setWall, currentBlock, setBlock, setGameUpdate) {
    if (ctx2D) {
        clearCanvas(ctx2D, canvas)
        blockLoop(ctx2D, canvas, wall, setWall, currentBlock, setBlock, setGameUpdate)
